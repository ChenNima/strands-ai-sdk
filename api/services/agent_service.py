...
def save_user_message(conversation_uuid: UUID, message: ClientMessage) -> None:
    ...
    try:
        ...
        if message.parts:
            parts_data = [part.model_dump(exclude_none=True) for part in message.parts]
        ...
        if conversation_title:
            conv_stmt = select(Conversation).where(Conversation.uuid == conversation_uuid)
...
def create_agent_with_session(conversation_id: str, config_path: str = "api/config/default_agent.json"):
    ...
    return config_to_agent(config=config_path, session_manager=session_manager)
